...
def read_iSNVtable(iSNV_tableF):
    CiteLst = {}
    for VcfFl in open(iSNV_tableF).readlines():
        if "X.Posi" not in VcfFl and VcfFl != "\n":
            Tags = VcfFl.split("\n")[0].split("\t")
            Cite = Tags[0]
            CiteLst[int(Cite)] = ''        
    return CiteLst               
...
def main():
    ...
    small_altPosi = read_iSNVtable(iSNVtable)
...
################################################################################################################################
    ...
    iSNVtable      = os.path.abspath(options.iSNVtable)
    out_Path       = os.path.abspath(options.out_Path)
    ONT_PosiIndex_step = int(options.ONT_PosiIndex_step)
    FiltPosi_ONTsnvsType_F  = out_Path + "/2.read-haplotype.txt"
    print(FiltPosi_ONTsnvsType_F)
    FiltPosi_ONTsnvsType_stat_F = out_Path + "/2.read-haplotype.Stat.txt"
    ...
    if (not os.path.exists(iSNVtable)):
